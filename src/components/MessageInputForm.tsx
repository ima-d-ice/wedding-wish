...
const MessageInputForm = () => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      await addDoc(collection(db, "mail_queue"), {
        to: email.trim(),
        message: {
          subject: `Thank You for Your Message, ${author.trim()}!`,
          html: `<p>Dear ${author.trim()},</p><p>Thank you for sharing your heartfelt message! It has been received and is now displayed.</p><p>Warmly,</p><p>The Event Team</p>`,
        },
      });
      console.log("Email task added to queue and message posted.");
      toast.success("Message sent and posted! Thank you!"); 
  ...
  return (
    ...
    >
      ...
      <div>
        <label htmlFor="author" className="block text-sm font-semibold text-sky-200 mb-1.5">
          Your Name
        </label>
        <input
          type="text"
          ...
          required
          className="mt-1 block w-full px-4 py-2.5 rounded-lg bg-slate-800/60 border border-sky-700/70 focus:border-sky-500 focus:bg-slate-700/70 focus:ring-2 focus:ring-sky-500/50 text-sky-50 placeholder-sky-400/60 shadow-sm transition-all duration-300 outline-none disabled:opacity-50"
          placeholder="e.g., IceCream Lover"
          disabled={isSubmitting}
        />
      </div>
      <div>
